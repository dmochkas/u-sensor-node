...
int schc_accept_message(const schc_message_t* msg_in, schc_message_t* msg_out) {
    ...
    for (int i = 0; i < lu_table.n_entries; ++i) {
        ...
        if ((e->req_msg.rule_id & target_mask) == target_id) {
            if (e->get_resp_payload != NULL) {
                size_t pl_size;
                int res = e->get_resp_payload(e->resp_msg.payload, SCHC_MAX_PAYLOAD_SIZE, &pl_size);
                if (res < 0) {
                    break;
                }
            ...
            if (e->get_req_payload != NULL) {
                size_t pl_size;
                int res = e->get_req_payload(e->req_msg.payload, SCHC_MAX_PAYLOAD_SIZE, &pl_size);
                if (res < 0) {
                    break;
                }
                e->req_msg.pl_size_bits = BYTES_TO_BITS(pl_size);
            }
            target = &e->req_msg;
            ret = 2;
        } else {
